...
<form action="{{url('admin/template/doEdit')}}" method="post" id="form" enctype="multipart/form-data">
    @csrf
    <ul class="index_ul">
        ...
        <li class="index_line">
            ...
            <input class="index_value" type="text" name="nickname" value="{{$data==''?'':$data->nickname}}">
        </li>
        <li class="index_line">
            <span class="index_name">地址</span> :
            <input class="index_value" type="text" name="name" value="{{$data==''?'':$data->name}}">
        </li>
    </ul>
    <textarea style="display: none" class="contextLocal" name="contextLocal" cols="30" rows="10"></textarea>
...
{{--            {!! $data==''?'':$data->context !!}--}}
    ...
    <div style="text-align: center">
        ...
        <a class="back" href="/admin/template/info?type={{$type}}">取消</a>
    </div>
</form>
</body>
<script>
    ...
    $('#submit').click(function(){
        $('#textarea').html(UE.getEditor('editor').getPlainTxt().replace('<pre>','').replace('</pre>',''))
        $('.contextLocal').val(UE.getEditor('editor').getPlainTxt().replace('<pre>','').replace('</pre>',''))
        $('#form').submit()
    })
</script>
</html>
