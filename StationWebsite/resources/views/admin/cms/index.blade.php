...
<div class="add_banner">
    <a href="/admin/cms/edit?table={{$table}}">添加{{$table=='news'?'新闻':'公告'}} ></a>
</div>
...
<table width="80%" class="pure-table pure-table-bordered">
    ...
    @foreach($data as $item)
        <tr>
            ...
            <td>
                <a href="/admin/action/delete?id={{$item->id}}&table={{$table}}&url=/admin/cms/info?table={{$table}}">删除</a>
                <a href="/admin/cms/edit?id={{$item->id}}&table={{$table}}">修改</a>
                <a href="/admin/action/change_status?table={{$table}}&id={{$item->id}}&status={{$item->status}}&url=/admin/cms/info?table={{$table}}">
                    {{$item->status==0?'禁用':'启用'}}
                </a>
            </td>
        </tr>
