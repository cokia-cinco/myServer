...
<div class="add_banner">
    <a href="/admin/list/viewEdit?table={{$table}}&tid={{$tid}}&type={{$type}}">添加 ></a>
    <a class="back" href="/admin/list/info?table=type_of_list&type={{$type}}">上一级</a>
</div>
...
<table width="80%" class="pure-table pure-table-bordered">
    ...
    @foreach($list as $item)
        <tr>
            ...
            <td>{{$item->sort}}</td>
            <td>
                <a href="/admin/action/delete?id={{$item->id}}&table=list&url=/admin/list/view?id={{$tid}}&type={{$type}}">删除</a>
                <a href="/admin/list/viewEdit?id={{$item->id}}&table=list&tid={{$tid}}&type={{$type}}">修改</a>
            </td>
        </tr>
    @endforeach
    </tbody>
